...
# ============================================================
# Script purpose:
# This script reads the raw EBSCOhost search results file,
...
#
# Input:
# data/0_initial_search/ebscohost.csv
#
# Output:
...
input_file  <- file.path(proj_root, "data", "0_initial_search", "ebscohost.csv")
output_file <- file.path(proj_root, "data", "1_parsed_databases", "parsed_ebscohost.csv")
...
# STEP 3: IMPORT THE RAW EBSCOHOST FILE ===============================
...
## 3b) Read the raw EBSCOhost export -------------------------------
raw_ebscohost <- read_csv(
  file      = input_file,
  guess_max = Inf,
...
ebscohost <- raw_ebscohost %>%
  mutate(
    year = year(ymd(publication_date)),
...
ebscohost <- ebscohost %>%
  filter(
    !(study_id == "Ray_Welch_Lawton_etal_2006" & type == "analytic"),
    !(study_id == "Gao Qingzhu_Kang Muyi_Xu Hongmei_etal_2010")
  )
...
## 6b) Write the cleaned EBSCOhost file ----------------------------
write_csv(
  x    = ebscohost,
  file = output_file,
  na   = ""
...
qc_summary <- tibble(
  ...
  value = c(
    ...
    nrow(raw_ebscohost),
    nrow(ebscohost),
    n_distinct(ebscohost$study_id, na.rm = TRUE),
    sum(is.na(ebscohost$study_id) | ebscohost$study_id == ""),
    sum(is.na(ebscohost$year)),
    sum(is.na(ebscohost$doi) | ebscohost$doi == ""),
...
cat("\nParsed EBSCOhost export complete.\n")
